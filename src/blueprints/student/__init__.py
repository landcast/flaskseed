...
def get_enter_room_url():
    ...
    with session_scope(db) as session:
        ...
        url = live_service.enter_room(getattr(g, current_app.config['CUR_USER'])['username'],courseclassroom.room_id,getattr(g, current_app.config['CUR_USER'])['nickname'],ClassroomRoleEnum.STUDENT.value,ClassroomDeviceEnum.PC.value)
